                                       ...
                                       unsigned char *dst)
{
    ...
    if (md_alg != MBEDTLS_MD_NONE) {
        ...
        if (hashlen != md_size) {
            mbedtls_printf(" hashlen is not md size \n");
            return MBEDTLS_ERR_RSA_BAD_INPUT_DATA;
        }
        ...
        if (nb_pad < 10 + hashlen + oid_size) {
            mbedtls_printf(" bounds 1 \n");
            return MBEDTLS_ERR_RSA_BAD_INPUT_DATA;
        }
        nb_pad -= 10 + hashlen + oid_size;
    } else {
        if (nb_pad < hashlen) {
            mbedtls_printf(" bounds 2 \n");
            return MBEDTLS_ERR_RSA_BAD_INPUT_DATA;
        }
